defmodule Croc.PipelinesTest.Games.Monopoly.Downgrade do

  use ExUnit.Case
  alias Croc.Games.Monopoly.{Event, Player, Lobby}
  alias Croc.Games.Monopoly
  alias Croc.Pipelines.Games.Monopoly.Downgrade
  ...
  test "should downgrade successfully", %{ game: game } do
    ...
    cards =
      ...
      |> Enum.map(fn c ->
        unless c.monopoly_type != card.monopoly_type do
          c
          |> Map.put(:owner, player.player_id)
          |> Map.put(:upgrade_level, c.max_upgrade_level)
        else
          c
        end
      end)
    ...
    game = args.game
    player = Enum.at(game.players, 0)
    assert player.balance == old_balance + card.upgrade_cost
